...
def render_hour_card(hour, day_color, day_bg):
    ...
    """
    final_score = hour['final_score'] # Still used for colors
    ...
    # --- MODIFICATION START ---
    # Get the raw score and format it for display
    ...
    try:
        ...
        for period in periods:
            try:
                ...
                if parsed_hour == 'N/A': continue
                
                hour_num = parsed_hour
                if hour_num == 0: formatted_hour = "12:00 AM"
                elif hour_num < 12: formatted_hour = f"{hour_num}:00 AM"
                elif hour_num == 12: formatted_hour = "12:00 PM"
                else: formatted_hour = f"{hour_num - 12}:00 PM"
                ...
                weather_entry = {
                    'Hour': formatted_hour, 'HourNum': hour_num, 'Temp': temp,
                    'Wind': wind, 'Precip': precip, 'Humidity': humidity,
                    'Forecast': forecast, 'dewpoint_fahrenheit': dewpoint,
                ...
                if 'TODAY' in day_category.upper():
                    ...
                    if raw_start_time:
                        ...
                        if dt.date() == today_date: today_data.append(weather_entry)
                        elif dt.date() == tomorrow_date: tomorrow_data.append(weather_entry)
        ...
        logger.error(f"Fatal error in parse_json_weather_data_corrected: {e}")
        return {'today': [], 'tomorrow': []}
...
def calculate_rwi_score(temperature, humidity, wind_speed, precipitation, forecast, dewpoint_fahrenheit, hour_data=None):
    """
    Calculate RWI using enhanced solar data from weather server when available
    """
    try:
        # Parse inputs (existing logic)
        ...
        # Calculate heat index (existing logic)
        ...
        # THERMAL COMFORT SCORE (unchanged)
        if heat_index <= 65:
            S_H = 5
        elif heat_index <= 75:
            S_H = 4.5 - (heat_index - 65) * 0.05
        elif heat_index <= 85:
            S_H = 4 - (heat_index - 75) * 0.2
        else:
            S_H = max(1, 4 - (heat_index - 75) * 0.2)
        ...
        # ENHANCED SOLAR CONDITIONS - Use server data when available
        if hour_data and 'solar_score' in hour_data:
            # Use pre-calculated solar score from enhanced weather server
            S_S = float(hour_data['solar_score'])
            solar_source = "enhanced_server"
            logger.info(f"RWI: Using enhanced solar score {S_S} from server (phase: {hour_data.get('solar_phase', 'unknown')})")
        else:
            # Fallback to original forecast-based logic
            S_S = get_fallback_solar_score(forecast, temp, heat_index)
            solar_source = "fallback"
            logger.info(f"RWI: Using fallback solar score {S_S}")
        
        # Clamp solar score to valid range
        S_S = min(5, max(1, S_S))
        
        # WIND SCORE (unchanged from previous fixes)
        if wind <= 1:
            S_W = 3.0
        elif wind <= 5:
            S_W = 4.5
            ...
            S_W = max(2, 5 - (wind - 20) * 0.1)
        
        # PRECIPITATION SCORE (unchanged)
        if precip < 5:
            S_P = 5
        elif precip < 15:
            S_P = 4.5
            ...
            S_P = max(1, 3 - (precip - 50) * 0.04)
        
        # DEWPOINT COMFORT (unchanged)
        if dewpoint <= 45:
            S_D = 5
        elif dewpoint <= 55:
            S_D = 4.5
            ...
            S_D = max(1, 3 - (dewpoint - 65) * 0.2)
        
        # WEIGHTED COMBINATION (unchanged)
        RWI = (0.40 * S_H + 0.15 * S_S + 0.20 * S_W + 0.10 * S_P + 0.15 * S_D)
        ...
        return {
            'rwi_score': 3.0, 'rating': 3, 'heat_index': float(temperature) if temperature != "N/A" else 70.0,
            'components': {'thermal_comfort': 3.0, 'solar_conditions': 3.0, 'wind': 3.0, 'precipitation': 3.0, 'dewpoint_comfort': 3.0},
            'solar_source': 'error_fallback'
        }

def get_fallback_solar_score(forecast, temperature, heat_index):
    """
    Fallback solar scoring when enhanced server data isn't available.
    This is the old time-based logic as backup.
    """
    forecast_lower = str(forecast).lower()
    
    # Without actual solar data, assume reasonable defaults
    if any(word in forecast_lower for word in ['sunny', 'clear', 'fair']):
        if heat_index > 70:
            solar_penalty = min(0.8, (heat_index - 70) * 0.02)
            return 5 - solar_penalty * 5
        else:
            return 5.0
        ...
        if heat_index > 75:
            return min(5.5, 5 + 0.5)  # Bonus for cloud cover when hot
        else:
            return 5.0
    else:
        return 5.0
...
def score_hour_with_scientific_approach(hour_data, aqi_value=None):
    """
    Enhanced scoring with solar-aware RWI integration and corrected overrides.
    """
    ...
    # Log enhanced solar data if available
    if 'solar_score' in hour_data:
        logger.info(f"ENHANCED SOLAR: score={hour_data['solar_score']}, phase={hour_data.get('solar_phase')}, explanation={hour_data.get('solar_explanation', 'N/A')}")
    
    # Use RWI for both raw and final (rounded) score
    ...
    # Temperature safety overrides now apply to BOTH scores
    if temp >= 90:
        final_score = 1
        raw_score = 1  # --- ADD THIS LINE ---
        logger.info("Applied temp>=90 override, final_score=1")
    elif temp >= 85:
        final_score = 2
        raw_score = 2  # --- ADD THIS LINE ---
        logger.info("Applied temp>=85 override, final_score=2")
    
    # Enhanced heat stress calculation using solar phase if available
    heat_stress_level = calculate_enhanced_heat_stress(temp, dewpoint, hour_data)
    
    # Generate solar-aware recommendations
...
def calculate_enhanced_heat_stress(temp, dewpoint, hour_data):
    """
    Calculate heat stress considering actual solar phase from server data
    """
    if temp == "N/A" or dewpoint == "N/A":
        return "Unknown"
    
    # Use enhanced solar phase if available
    ...
    if is_nighttime:
        # Nighttime thresholds - much higher since no solar load
        ...
        # Daytime thresholds (original logic)
...
def generate_solar_aware_recommendation(temp, dewpoint, wind, forecast, final_score, raw_score, hour_data):
    """
    Generate recommendations using enhanced solar data and nuanced headers.
    """
    recommendations = []
    ...
    if temp >= 90:
        ...
        return "DANGEROUS CONDITIONS: " + " • ".join(recommendations)
    elif temp >= 85:
        ...
        recommendations.append("Cool conditions - extend warm-up time")
    
    # Enhanced solar guidance using server data
    solar_phase = hour_data.get('solar_phase', 'unknown')
    ...
    # Humidity and wind (existing logic)
    if dewpoint >= 65:
    ...
    # --- MODIFIED LOGIC FOR HEADER ---
    # Overall assessment with the new rule
    if final_score >= 4:
    ...
    # recommendations.append("Monitor your body's response and adjust as needed")
...
def _generate_rwi_recommendation(temp, dewpoint, wind, precip, forecast, aqi_value, final_score, overall_risk):
    ...
    if temp >= 90:
        recommendations.append("🌡️ DANGEROUS HEAT: Avoid outdoor running. Heat stroke risk is very high.")
        recommendations.append("🏠 Move indoors or postpone until temperatures drop below 85°F.")
    elif temp >= 85:
        recommendations.append("🔥 HIGH HEAT STRESS: Easy pace only with frequent breaks.")
        recommendations.append("💧 Hydrate every 10-15 minutes. Pre-cool with cold fluids.")
        recommendations.append("🌳 Seek shaded routes and avoid direct sun exposure.")
    elif temp >= 80:
        recommendations.append("⚠️ HEAT STRESS DEVELOPING: Reduce intensity for runs longer than 30 minutes.")
        recommendations.append("💧 Increase hydration frequency to every 15-20 minutes.")
    elif temp >= 75:
        recommendations.append("🌡️ WARM CONDITIONS: Monitor effort level and hydration needs.")
    elif temp >= 50:
        recommendations.append("👍 GOOD TEMPERATURE: Comfortable for all training intensities.")
    elif temp >= 32:
        recommendations.append("🧥 COOL CONDITIONS: Allow extra time for warm-up.")
    else:
        recommendations.append("🥶 COLD CONDITIONS: Layer appropriately and protect extremities.")

    # Dewpoint/humidity guidance
    if dewpoint > 70:
        recommendations.append(f"💦 OPPRESSIVE HUMIDITY: Dewpoint {dewpoint}°F severely impairs cooling.")
    elif dewpoint > 65:
        recommendations.append(f"💧 HIGH HUMIDITY: Dewpoint {dewpoint}°F increases heat stress significantly.")
    elif dewpoint > 60:
        recommendations.append(f"💧 NOTICEABLE HUMIDITY: Dewpoint {dewpoint}°F may affect comfort.")

    # Solar/cloud guidance
    if 'sunny' in forecast or 'clear' in forecast:
        if temp > 75:
            recommendations.append("☀️ DIRECT SUN: Adds 10-15°F to effective temperature. Seek shade.")
        else:
            recommendations.append("☀️ SUNNY CONDITIONS: Pleasant for cooler temperatures.")
    elif 'cloudy' in forecast or 'overcast' in forecast:
        if temp > 75:
            recommendations.append("☁️ CLOUD COVER: Provides beneficial relief from direct sun.")

    # Wind guidance
    if wind > 20:
        recommendations.append(f"💨 STRONG WIND: {wind} mph affects pacing. Run by effort, not speed.")
    elif wind > 15:
        recommendations.append(f"💨 MODERATE WIND: {wind} mph provides cooling but may affect pace.")
    elif 5 <= wind <= 15:
        recommendations.append(f"🌬️ BENEFICIAL WIND: {wind} mph provides excellent cooling.")

    # Precipitation guidance  
    if precip > 50:
        recommendations.append(f"🌧️ HEAVY RAIN LIKELY: {precip}% chance. Significant slip hazard.")
    elif precip > 20:
        recommendations.append(f"🌦️ RAIN POSSIBLE: {precip}% chance. Adjust pace for wet conditions.")

    # Air quality guidance
    if aqi_value and aqi_value != "N/A":
        if aqi_value > 150:
            recommendations.append(f"🚫 POOR AIR QUALITY: AQI {aqi_value}. Consider indoor alternatives.")
        elif aqi_value > 100:
            recommendations.append(f"⚠️ MODERATE AIR QUALITY: AQI {aqi_value}. Sensitive individuals use caution.")

    # Overall assessment based on final score
    if final_score == 5:
        header = "✅ EXCELLENT CONDITIONS"
        recommendations.insert(0, "Perfect conditions for any training intensity!")
    elif final_score == 4:
        header = "👍 VERY GOOD CONDITIONS" 
        recommendations.insert(0, "Great conditions with minor considerations.")
    elif final_score == 3:
        header = "⚠️ FAIR CONDITIONS"
...
def _get_running_recommendation_enhanced(hour_data, aqi_value, composite_score):
    ...
    if overall_risk == OPTIMAL:
        ...
        header = "❗ High Risk - Adjust Your Run"
        ...
        header = "🛑 Dangerous Conditions - Avoid Outdoor Running"
    ...
    safety_alerts = []
    if any(term in forecast for term in ['thunderstorm', 'thunder', 'lightning']):
        safety_alerts.append("⚡ Lightning Risk: Seek indoor shelter immediately if thunder is heard.")
    
    if temp != "N/A" and temp >= 90 and dewpoint != "N/A" and dewpoint >= 70:
        safety_alerts.append("🌡️ Heat Emergency Risk: Symptoms include confusion, hot dry skin, rapid pulse.")
    
    if temp != "N/A" and temp <= 20 and wind != "N/A" and wind >= 15:
        safety_alerts.append("🥶 Frostbite Risk: Cover all exposed skin; watch for numbness or white/grayish skin.")
    
    if aqi_value != "N/A" and aqi_value is not None and aqi_value >= 200:
        safety_alerts.append("😷 Health Emergency: Air quality at dangerous levels affecting everyone.")
...
def format_weather_line_with_na(hour):
    """Format a weather line handling N/A values and nighttime icons properly."""
    ...
    if hour['Precip'] != "N/A":
        if hour['Precip'] > 30:
            ...
            # --- UPDATED LOGIC ---
            # Checks the solar phase to show a moon at night for 0% precipitation
...
# In multi_agent_runner.py, add this new function

def _generate_compact_html_analysis(scored_data: list, city: str) -> str:
    """
    Generates a compact HTML forecast with final styling, including
    colored dates and a global recommendation message.
    """
    if not scored_data:
        return f"<div style='color: red;'>No weather data available for {city}.</div>"

    # --- Header with the new global message ---
    ...
    for hour in scored_data:
        # --- Robust Date Logic and Color Selection ---
        clean_date_for_line = ""
        date_color = "#4169E1" # Default/Today color (a nice blue)
        try:
            day_category = hour.get('day_category', 'Date Unknown')
            if 'TOMORROW' in day_category.upper():
                date_color = "#9932CC" # Tomorrow color (a nice purple)
        ...
        # --- Data Extraction & Icon Logic (no changes) ---
        ...
        weather_icon = '🌤️'
        if temp != 'N/A' and temp >= 90: weather_icon = '🌡️'
        elif 'thunder' in forecast_str: weather_icon = '⛈️'
        elif solar_phase == 'night': weather_icon = '🌙'
        elif 'sunny' in forecast_str or 'clear' in forecast_str: weather_icon = '☀️'
        elif 'partly' in forecast_str: weather_icon = '🌤️'
        elif 'cloudy' in forecast_str: weather_icon = '☁️'
        elif 'rain' in forecast_str: weather_icon = '🌧️'
        
        # --- Color Coding & RWI Score Emoji Logic (no changes) ---
        score_colors = {
        ...
        if raw_score >= 4.5: score_emoji = "🏃🏃🏃"
        elif raw_score >= 4.0: score_emoji = "🏃🏃"
        elif raw_score >= 3.5: score_emoji = "🏃"
        elif raw_score >= 3.0: score_emoji = "🤔 🏃"
        elif raw_score >= 2.0: score_emoji = "🔶"
        else: score_emoji = "🛑"
        ...
        else: # Covers scores >= 1.0 up to 1.99
            status_display = "🚫 UNSAFE"
        
        # --- Main Weather & Score Line ---
        ...
        # --- Full Recommendation Formatting (no changes) ---
        rec_points_html = ""
...
def _generate_concise_text_analysis(scored_data: list, city: str) -> str:
    """
    Generates a very concise, single-line-per-hour text forecast.
    """
    if not scored_data:
        return f"No weather data available for {city} in the selected time range."

    # --- Helper dictionaries for emojis ---
    heat_emojis = {'Minimal': '🟢', 'Low': '🟢', 'Moderate': '🟠', 'High': '🔴', 'Extreme': '🔴', 'Unknown': '⚪'}
    aqi_emojis = {'Good': '🟢', 'Moderate': '🟠', 'Unhealthy for Sensitive': '🟠', 'Unhealthy': '🔴', 'Very Unhealthy': '🔴', 'Hazardous': '💀', 'Unknown': '⚪'}

    # --- Main processing loop ---
    output_lines = []
    
    # Get the date from the first entry to use as a header
    first_hour = scored_data[0]
    day_category = first_hour.get('day_category', 'Date Unknown')
    
    # Attempt to parse a clean date format
    try:
        # Example: "TODAY-Sep 23, Tuesday" -> "Tue, Sep 23"
        date_part = day_category.split('-')[1] # "Sep 23, Tuesday"
        day_of_week = date_part.split(', ')[1][:3] # "Tue"
        month_day = date_part.split(', ')[0] # "Sep 23"
        clean_date = f"{day_of_week}, {month_day}"
    except IndexError:
        clean_date = day_category

    current_date = clean_date

    for i, hour in enumerate(scored_data):
        # --- Date Header Logic ---
        # Check if the day has changed from the previous entry
        try:
            next_day_category = hour.get('day_category', 'Date Unknown').split('-')[1]
            next_day_of_week = next_day_category.split(', ')[1][:3]
            next_month_day = next_day_category.split(', ')[0]
            next_clean_date = f"{next_day_of_week}, {next_month_day}"
            if next_clean_date != current_date:
                current_date = next_clean_date
                # Add a blank line and a new date header when the date changes
        ...
        heat_level = hour.get('heat_stress_level', 'Unknown')
        heat_emoji = heat_emojis.get(heat_level, '⚪')
        
        aqi_level = hour.get('aqi_category', 'Unknown')
        aqi_emoji = aqi_emojis.get(aqi_level, '⚪')
        
        score = hour.get('final_score', 'N/A')

        # --- Concise Recommendation Logic ---
        recommendation = ""
        if score == 5:
        ...
        # Specific override based on your rule
        if temp != 'N/A' and temp >= 76 and dew_point != 'N/A' and dew_point < 60:
             recommendation = "      ⚠️ Temp high but dew point is low. Hydrate well."

        # --- Line Assembly ---
        line1 = (
            f"{current_date} {time_str:<5} | 🌡️{temp}°F | DewPt {dew_point}°F | "
            f"Wind {wind}mph | 💧{humidity}% | 🌧️{precip}% | "
            f"Heat {heat_emoji}{heat_level} | AQI {aqi_emoji}{aqi_level} | "
            f"Score: {score}/5"
        )
...
def get_enhanced_solar_adjustment_for_rwi(hour_data):
    """
    Use the enhanced solar data in your RWI calculations
    """
    solar_score = hour_data.get('solar_score', 5.0)  # Default to neutral if not available
    solar_phase = hour_data.get('solar_phase', 'unknown')
    is_solar_time = hour_data.get('is_solar_time', False)
    
    # The solar_score is already calculated based on forecast + time + temperature
    # Just use it directly in your RWI calculation
    return solar_score, f"Solar phase: {solar_phase}"
...
def _generate_filtered_analysis(
    ...
    try:
        ...
        for hour in scored_hours:
            ...
            stars = "★" * risk_based_stars + "☆" * (5 - risk_based_stars)
...
# In multi_agent_runner.py, add this new tool

@tool
def get_city_from_zipcode(zip_code: str) -> str:
    """
    Converts a 5-digit US zip code to a 'City, State' format.
    Returns the original input if it's not a valid zip code or if the lookup fails.
    """
    if not re.match(r'^\d{5}$', zip_code):
        return zip_code # It's not a zip code, return as is (e.g., "Boston, MA")
    ...
    try:
        ...
        logger.warning(f"Failed to convert zip code {zip_code}: {e}. Using the original value.")
        return zip_code # Return original zip on failure
...
def schedule_daily_email_report(
    ...
    """
    Schedules a daily email report for a specific time and date range.
    """
    ...
    def job():
        ...
        try:
            analysis_html = run_agent_workflow(
                ...
                output_format="compact_html"
            )
            subject = f"Your Daily Running Forecast for {location}"
            email_body = f"<!DOCTYPE html><html><body>{analysis_html}</body></html>"
...
# In multi_agent_runner.py, replace the run_agent_workflow function

def run_agent_workflow(
    location: str,
    time_windows: dict,
    output_format: str = "compact_html"
) -> str:
    """
    Main workflow that handles zip code conversion and multiple time windows.
    'time_windows' is a dict like {'today_1': ('06:00', '10:00'), 'tomorrow_1':...}
    """
    try:
        # Step 1: Convert location if it's a zip code
        ...
        # Step 3: Filter and score data for all provided windows
        all_scored_hours = []
        seen_hours = set() # To prevent duplicates if windows overlap

        def time_to_hour(time_str): return int(time_str.split(':')[0])

        for window_key, times in time_windows.items():
            if not times or not times[0] or not times[1]: continue # Skip empty windows
            ...
            # Simple AQI parsing (can be enhanced later)
            ...
            for hour in scored:
                # Use a unique key to identify an hour to avoid duplicates
        ...
        if not all_scored_hours:
            return f"<div style='color: #F44336;'>No data available for the selected time windows in {city}.</div>"

        all_scored_hours.sort(key=lambda x: (x.get('day_category', 'z'), x.get('HourNum', 0)))
        
        if output_format.lower() == "compact_html":
            return _generate_compact_html_analysis(all_scored_hours, city)
        # Add other output formats here if needed
        else:
            return _generate_concise_text_analysis(all_scored_hours, city)
        
    except Exception as e:
        logger.error(f"Workflow error for location '{location}': {e}")
        ...
        try:
            ...
            if user_input:
                ...
                result = run_agent_workflow(
                    ...
                    tomorrow_end="22:00",
                    output_format="concise"  # Change to "html" or "text" as needed
                )
                print("Result:")
                print("=" * 50)
                print(result)
