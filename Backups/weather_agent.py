...
@tool
def get_weather_forecast_from_server(city: str, granularity: str = 'daily') -> str:
    """
    Gets the weather forecast for a city by calling the MCP server.
    The 'granularity' parameter can be either 'daily' or 'hourly'.
    ...
    # Update payload to include granularity
    payload = {"city": city, "granularity": granularity}
    headers = {"Content-Type": "application/json"}
    try:
        print(f"\n--- Calling Weather Tool Server for city: {city}, granularity: {granularity} ---\n")
...
if __name__ == "__main__":
    ...
    print("You can also ask for the hourly forecast (e.g., 'what is the hourly weather in NYC?')")
