...
<div class="container">
    ...
    <label for="vita_avatar">Your Vita Avatar</label>
    <input type="text" id="vita_avatar" name="vita_avatar" placeholder="e.g., Morning Warrior, Comeback Runner, Weekend Champion..." value="The Determined Runner">
</div>

<div class="form-group">
    <label for="vita_description">Vita Description</label>
    <div style="display: flex; gap: 8px; align-items: center;">
        <textarea id="vita_description" name="vita_description" rows="4" placeholder="Describe who you are or who you want to become through this program...">I want to be someone who wakes up excited about how positive I feel about the day</textarea>
        <button type="button" onclick="startVoiceInput('vita_description')" style="padding: 8px 12px; background: #2196F3; color: white; border: none; border-radius: 5px; cursor: pointer; height: fit-content;">
            🎤 Voice
        </button>
    </div>
</div>
                <div class="form-row-3">
                    <div class="form-group">
                        ...
                        <select id="age" name="age">
                            ...
                            <option value="{{ age }}" {% if age == 30 %}selected{% endif %}>{{ age }}</option>
                            {% endfor %}
                        </select>
                    </div>
                    <div class="form-group">
                        ...
                        <div style="display: flex; gap: 8px;">
                            <select id="height_feet" name="height_feet" style="flex: 1;">
                                ...
                                {% for ft in range(4, 8) %}
                                <option value="{{ ft }}" {% if loop.last %}selected{% endif %}>{{ ft }} ft</option>
                                {% endfor %}
                            </select>
                            <select id="height_inches" name="height_inches" style="flex: 1;">
                                <option value="">Inches</option>
                                {% for inch in range(0, 12) %}
                                <option value="{{ inch }}" {% if loop.last %}selected{% endif %}>{{ inch }} in</option>
                                {% endfor %}
                            </select>
                        </div>
                    </div>
                    <div class="form-group">
                        ...
                        <input type="number" id="weight" name="weight" value="160" min="80" max="400" placeholder="lbs">
                    </div>
                </div>
                <div class="form-row">
                    <div class="form-group"><label for="gender">Gender</label><select id="gender" name="gender"><option value="">Select...</option><option value="male" selected>Male</option><option value="female">Female</option><option value="other">Other</option></select></div>
                </div>
                <div class="form-group">
                    <label for="mobility_restrictions">Mobility Restrictions</label>
                    <div style="display: flex; gap: 8px; align-items: center;">
                        <textarea id="mobility_restrictions" name="mobility_restrictions" rows="3" placeholder="Describe any mobility issues or physical limitations...">Cannot run above 6 mph</textarea>
                ...
                <div class="form-group">
                    ...
                    <div style="display: flex; gap: 8px; align-items: center;">
                        <textarea id="health_conditions" name="health_conditions" rows="3" placeholder="E.g., No health concerns, Diabetes, Back pain, Asthma, Heart condition...">Back pain</textarea>
                ...
                <div class="form-group">
                    ...
                    <div style="display: flex; gap: 8px; align-items: center;">
                        <textarea id="dietary_restrictions" name="dietary_restrictions" rows="3" placeholder="E.g., Vegan, Gluten-free, Dairy-free, Nut allergies...">Vegetarian, Allergic to bananas</textarea>
                ...
                <div class="form-group">
                    ...
                    <div style="display: flex; gap: 8px; align-items: center;">
                        <textarea id="other_details" name="other_details" rows="3" placeholder="Any other information that would help personalize your training plan...">Had surgery on my right leg 4 weeks back</textarea>
                ...
                <div class="form-row">
                    <div class="form-group">
                        ...
                        <select id="run_plan" name="run_plan">
                            ...
                            <option value="athletic_goal" selected>3. Athletic achievement goal</option>
                        </select>
                    </div>
                ...
                <div class="form-row">
                    <div class="form-group">
                        ...
                        <select id="plan_period" name="plan_period">
                            ...
                            {% for i in range(1, 21) %}<option value="week_{{ i }}" {% if loop.last %}selected{% endif %}>Week {{ i }}</option>{% endfor %}
                        </select>
                    </div>
                    <div class="form-group">
                        <label for="plan_display">Plan Display Options</label>
                        <select id="plan_display" name="plan_display">
                            <option value="full_plan" selected>Show Full Plan</option>
                ...
                <div class="form-group">
                    ...
                    <textarea id="additional_details" name="additional_details" rows="8" placeholder="Specify dietary restrictions, other preferences, other health conditions, or anything else we should know to personalize your plan.">Want to reduce 20 lbs</textarea>
                </div>
            </div>
        </details>
        <h3>Forecast Options</h3>
        <div class="form-group"><label for="location">City or Zip Code</label><input type="text" id="location" name="location" value="08902" required placeholder="e.g., Boston, MA or 02101"></div>
        <h3>Today's Windows</h3>
        <details open><summary>My Availability Time Window 1 (Today)</summary><div class="window-content"><div class="time-input-group"><select name="today_1_start" class="start-time" data-day="today"><option value="">Start</option>{% for val, disp in today_hours %}<option value="{{ val }}" {% if val == '18:00' %}selected{% endif %}>{{ disp }}</option>{% endfor %}</select><span>to</span><select name="today_1_end" class="end-time" data-day="today"><option value="">End</option>{% for val, disp in today_hours %}<option value="{{ val }}" {% if val == '22:00' %}selected{% endif %}>{{ disp }}</option>{% endfor %}</select></div></div></details>
        ...
        <details><summary>My Availability Time Window 1 (Tomorrow)</summary><div class="window-content"><div class="time-input-group"><select name="tomorrow_1_start" class="start-time" data-day="tomorrow"><option value="">Start</option>{% for val, disp in tomorrow_hours %}<option value="{{ val }}" {% if val == '06:00' %}selected{% endif %}>{{ disp }}</option>{% endfor %}</select><span>to</span><select name="tomorrow_1_end" class="end-time" data-day="tomorrow"><option value="">End</option>{% for val, disp in tomorrow_hours %}<option value="{{ val }}" {% if val == '10:00' %}selected{% endif %}>{{ disp }}</option>{% endfor %}</select></div></div></details>
        ...
        <div class="form-group"><label for="email">Your Email Address</label><input type="email" id="email" name="email" value="sramana101@gmail.com" placeholder="Required for email actions"></div>
        <div class="schedule-options">
            <div><label for="schedule_time">Daily Email Time</label><select id="schedule_time" name="schedule_time">{% for hour in range(24) %}<option value="{{ '%02d:00'|format(hour) }}" {% if loop.last %}selected{% endif %}>{{ (datetime.strptime('%02d:00'|format(hour), '%H:%M')).strftime('%-I:%M %p') }}</option>{% endfor %}</select></div>
...
def index():
    ...
    # CRITICAL: Check mobile_view value
    mobile_view = form_data.get('mobile_view', ['false'])[0]
    
    import logging
    logging.info(f"=== DESKTOP ROUTE TRIGGERED ===")
    logging.info(f"Mobile view setting: {mobile_view}")
    ...
    try:
        ...
        logging.info(f"Is mobile result: {result.get('is_mobile', False)}")
        ...
        logging.error(f"Desktop workflow error: {e}", exc_info=True)
        return render_template_string(
            HTML_TEMPLATE,
            message=f'Error: {str(e)}',
            report_html=f'<div style="color: red;">Error: {str(e)}</div>',
            today_hours=today_hours,
            tomorrow_hours=tomorrow_hours,
            datetime=datetime,
            range=range
...
def mobile_index():
    ...
    if request.method == 'POST':
        ...
        # DEBUG LOGGING
        import logging
        logging.info(f"Form data received: {list(form_data.keys())}")
        logging.info(f"Mobile view: {form_data.get('mobile_view', ['false'])[0]}")
        ...
        try:
            ...
            # DEBUG LOGGING
            logging.info(f"Result keys: {list(result.keys())}")
            logging.info(f"Is mobile: {result.get('is_mobile', False)}")
            if 'card_data' in result:
                logging.info(f"Card data keys: {list(result['card_data'].keys())}")
                if 'days_plan' in result['card_data']:
                    logging.info(f"Days plan content length: {len(result['card_data']['days_plan'].get('content', ''))}")
